  ...
  <layout>
    <section class="container">
      ...
      <div class="actions" v-if="loading === false">
        <button @click="openModal" class="btn btn-primary btn-block">Edit</button>
        <button @click="handleDeleteTicket" class="mb-3 btn btn-danger btn-block">Delete</button>
      </div>
      <ticket-edit
        v-if="editModalState"
  ...
  methods: {
    ...
    async handleDeleteTicket() {
      if (confirm("Are you sure you want to delete this ticket")) {
        try {
          await axios.delete(
            `/api/customer/${this.id}?customerName=${this.customerName}`
          );
          this.$router.push("/tickets");
        } catch (err) {
          showToast(this.$toasted.show, "Something went wrong", "error");
        }
      }
    }
  },
  async created() {
    ...
    try {
      ...
      const res = await axios.get(
        `/api/customer/${this.id}?customerName=${customerName}`
      );
